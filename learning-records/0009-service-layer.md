...
- [ ] 第 10 课：全局异常处理（@RestControllerAdvice）
- [ ] 第 11 课+：参数校验（Bean Validation）、统一响应格式

下一课方向明确：第 9 课 createUser 抛出的 IllegalArgumentException 会让 Spring Boot 返回 500 HTML 错误页（Whitelabel Error Page），前端 axios 拿到 HTML 而非 JSON。第 10 课用 @RestControllerAdvice 全局异常处理，把所有异常统一转成规范的 JSON 错误响应。
